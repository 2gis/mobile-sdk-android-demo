...
- Исправлена ошибка, когда через `MapObjectManager` добавляется на карту маркер до полной загрузки изображения. Теперь устанавливается в качестве изображения прозрачное png 1x1 пока реальное не готово.
- Обновлен логотип 2ГИС + СБЕР на карте.
- При поисковом запросе для некоторых объектов возвращался region_id == 0, но ненулевой segment_id. Теперь такие объекты обрабатываются корректно.
...
- Обновлены общие стили:
  ...
  1. В редактор перенесены:
     ...
     На текущий момент не настраиваются в редакторе только выноски с информацией о маршруте, а также нельзя задать местоположение объектов `MapObjectManager`’а.
  ...
  Сейчас в редакторе подъезды отображаются неправильно, этого не нужно пугаться — в Mobile SDK они будут на месте!
