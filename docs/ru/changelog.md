...
- Теперь `CameraPositionPoint` учитывается при расчете позиции камеры `CameraPosition`.
- *Ломающее изменение*. Доработано отображение маркера текущего местоположения - теперь иконку с направлением можно рисовать всегда, независимо от режима навигации.
  ...
  В случае использования кастомных стилей, для корректной работы NSDK в редакторе стилей нужно поправить динамические объекты текущего местоположения следующим образом:
