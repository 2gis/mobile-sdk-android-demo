...
* `Model.routeInfo` переименовано в `Model.route`, `Model.currentRoutePosition` в `Model.routePosition`, вместо `Model.badLocation` теперь противоположное `Model.locationAvailable`.
