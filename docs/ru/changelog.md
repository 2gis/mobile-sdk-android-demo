...
- Изменена логика работы с ключами. В `assets` приложения нужно добавить файл `dgissdk.key`, который содержит информацию о ключе:
  ...
  - `service/properties/partner/oldKey` - старый ключ для работы со справочником.
  - `service/features` - возможности, которые доступны при использовании данного ключа.
  - `service/is_active` - активность ключа.
  - `service/updated_at` - дата последнего изменения данных в ключе.
  - `restrictions/app_id` - идентификатор приложения, в рамках которого доступно использование данного ключа.
  - `deactivation_time` - дата деактивации ключа.
...
- Исправление в отмене соединений у `Channel`.
...
- Методы `SearchQueryBuilder.setLocation` и `SuggestQueryBuilder.setLocation` удалены - значение автоматически берется из источника геопозиции.
- Метод `PackedSearchQuery.location` удален.
- Удалены `SearchResultMarkerObject` и `SearchResultMarkerSource` для генерализации маркеров поисковой выдачи.
- При создании `MyLocationMapObjectSource` используется `SmoothMyLocationController` вместо `RawMyLocationController`.
...
**Миграция с v2.x**
Map-версия и Full-версия:
  - Нужно обратится в техническую поддержку 2ГИС для получения нового ключа для использования MobileSDK. Полученный файл ключа `dgissdk.key` нужно добавить `assets`.
  - Обновить стили
